...
// countMintCnt
func (ec *EpochContext) countMintCnts(parent *types.Header) (mintcnt map[common.Address]int64, err error) {
	mintcnts := map[common.Address]int64{}

	epoch := parent.Time.Int64() / epochInterval
	candidateTrie := ec.DposContext.CandidateTrie()
	iterCandidate := trie.NewIterator(candidateTrie.NodeIterator(nil))
	existCandidate := iterCandidate.Next()
	if !existCandidate {
		return mintcnts, errors.New("no candidates")
	}
	for existCandidate {
		candidate := iterCandidate.Value
		candidateAddr := common.BytesToAddress(candidate)
		...
		_, ok := mintcnts[candidateAddr]
		if(!ok) {
			mintcnts[candidateAddr] = int64(0)
		}
		mintcnts[candidateAddr] = cnt	
		existCandidate = iterCandidate.Next()
	}
...
// countHistoryMintCnts
func (ec *EpochContext) countHistoryMintCnts(parent *types.Header) (mintcnt map[common.Address]int64, err error) {
	mintcnts := map[common.Address]int64{}

	genesis := ec.chain.GetHeaderByNumber(0)
	genesis_epoch := genesis.Time.Int64() / epochInterval
	epoch := parent.Time.Int64() / epochInterval
	if genesis_epoch == epoch {
		return ec.countMintCnts(parent)
	}
	for ;genesis_epoch <= epoch; genesis_epoch++{
		candidateTrie := ec.DposContext.CandidateTrie()
		iterCandidate := trie.NewIterator(candidateTrie.NodeIterator(nil))
		existCandidate := iterCandidate.Next()
		if !existCandidate {
		...
		for existCandidate {
			...
			candidateAddr := common.BytesToAddress(candidate)
			key := make([]byte, 8)
			binary.BigEndian.PutUint64(key, uint64(genesis_epoch))
			key = append(key, candidate...)
			cnt := int64(0)
			if cntBytes := ec.DposContext.MintCntTrie().Get(key); cntBytes != nil {
				cnt = int64(binary.BigEndian.Uint64(cntBytes))
