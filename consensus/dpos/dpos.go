...
import (
	...
	"github.com/gmc/gmcc/accounts"
	"github.com/gmc/gmcc/common"
	"github.com/gmc/gmcc/consensus"
	"github.com/gmc/gmcc/consensus/misc"
	"github.com/gmc/gmcc/core/state"
	"github.com/gmc/gmcc/core/types"
	"github.com/gmc/gmcc/crypto"
	"github.com/gmc/gmcc/crypto/sha3"
	"github.com/gmc/gmcc/ethdb"
	"github.com/gmc/gmcc/log"
	"github.com/gmc/gmcc/params"
	"github.com/gmc/gmcc/rlp"
	"github.com/gmc/gmcc/rpc"
	"github.com/gmc/gmcc/trie"
...
const (
	...
	epochInterval    = int64(blockInterval * maxValidatorSize * 30)// int64(86400)
	maxValidatorSize = 21
...
var (
	...
	frontierBlockReward  *big.Int = big.NewInt(2e+18) //big.NewInt(5e+18) // Block reward in wei for successfully mining a block
	byzantiumBlockReward *big.Int = big.NewInt(2e+18) //big.NewInt(3e+18) // Block reward in wei for successfully mining a block upward from Byzantium

	BlockRewardTime = int64(1590940800)
...
var (
	...
	errNotFoundParentBlock = errors.New("current block not found parent block")
...
func (d *Dpos) Prepare(chain consensus.ChainReader, header *types.Header) error {
	// if header.Number.Uint64() == 1 && d.config.GenesisTimestamp.Uint64() > uint64(time.Now().Unix()) {
	// 	for {
	// 		delay := time.Unix(int64(d.config.GenesisTimestamp.Uint64() - 2), 0).Sub(time.Now())
	// 		if delay <= time.Duration(0) {
	// 			log.Info("Ready for seal block", "time", time.Now())
	// 			break
	// 		} else if delay > time.Duration(blockInterval)*time.Second {
	// 			delay = time.Duration(blockInterval) * time.Second
	// 		}
	// 		log.Info("Waiting for seal block", "delay", common.PrettyDuration(time.Unix(int64(d.config.GenesisTimestamp.Uint64() - 2), 0).Sub(time.Now())))
	// 		select {
...
func AccumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if header.Time.Int64() < BlockRewardTime{
		blockReward = big.NewInt(0)
	}
	// spew.Dump("blockReward", blockReward)
...
// totalsupply 0.05%
func BecomeCandidateBalance() *big.Int{
	baseBalance, _ := new(big.Int).SetString("48000000000000000000000", 0)
	return baseBalance
}
