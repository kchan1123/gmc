...
func (self *worker) createNewWork() (*Work, error) {
	...
	if parent.Time().Cmp(new(big.Int).SetInt64(tstamp)) < -10 {
		tstamp = parent.Time().Int64() + 1
	}

	// if new(big.Int).SetInt64(tstamp).Cmp(parent.Time()) >= 10 {
	// 	tstamp = parent.Time().Int64() + 1
	// 	return nil, fmt.Errorf("current timestamp more than parent block timestamp")
	// }
