...
func (g *Genesis) ToBlock() (*types.Block, *state.StateDB) {
	...
	head := &types.Header{
		...
		Time:        g.Config.Dpos.GenesisTimestamp, //genesis block time is main net start time
	}
	if g.GasLimit == 0 {
...
func (g *Genesis) Commit(db ethdb.Database) (*types.Block, error) {
	...
	// fail create genesis block when genesis time is later than now 
	// if block.Header().Time.Cmp(big.NewInt(time.Now().Unix())) < 0 {
	// 	return nil, fmt.Errorf("genesis time is later than now ")
	// }
	config := g.Config
	if config == nil {
