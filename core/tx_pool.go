...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if tx.Type() == types.LoginCandidate {
		balance := pool.currentState.GetBalance(from)
		if balance.Cmp(dpos.BecomeCandidateBalance()) < 0 {
			return types.ErrInvalidbBecomeCandidateBalance
		}
	}
